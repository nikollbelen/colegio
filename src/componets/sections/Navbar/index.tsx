  ...
  return (
    ...
    >
      ...
      <div className="flex-grow flex justify-end items-center">
        ...
        <button
          onClick={toggleMenu}
          className="lg:hidden text-[#18212D] hover:text-[#18212D]"
        >
          <svg
            className="w-6 h-6"
            fill="none"
        ...
        <ul className="hidden lg:flex items-center gap-8 px-0 py-1">
          <li>
            <Link
              href="/"
              className="text-[#18212D] hover:text-[#18212D] font-medium"
            >
              {t("NB001")}
            </Link>
          </li>
          <li className="relative">
            <button
              ...
              className="text-[#18212D] hover:text-[#18212D] font-medium flex items-center"
            >
              {t("NB002")}
              <svg
            ...
            {isNosotrosOpen && (
              <div className="absolute top-full left-0 mt-8 w-48 bg-[var(--beige,#EEEBE7)] rounded-md shadow-lg border border-gray-200 z-50">
                <div className="py-1">
                  <Link
                    href="/historia"
                    className="block px-4 py-2 text-sm text-[#18212D] hover:bg-[#18212D]"
                  >
                    Historia
                  </Link>
                  <Link
      ...
      >
        <div className="p-6">
          <div className="flex justify-between items-center mb-8">
            ...
            <button
              onClick={toggleMenu}
              className="text-[#18212D] hover:text-[#18212D]"
            >
              <svg
                className="w-6 h-6"
                fill="none"
          ...
          <ul className="space-y-4">
            <li>
              <Link
                href="/"
                className="block text-[#18212D] hover:text-[#18212D] font-medium"
                onClick={toggleMenu}
              >
                {t("NB001")}
              </Link>
            ...
            <li>
              <button
                onClick={toggleNosotros}
                className="flex items-center justify-between w-full text-left text-[#18212D] hover:text-[#18212D] font-medium"
              >
                {t("NB002")}
                <svg
              ...
              {isNosotrosOpen && (
                <div className="pl-4 mt-2 space-y-2">
                  <Link
                    href="/historia"
                    className="block text-[#18212D] hover:text-[#18212D]"
                    onClick={toggleMenu}
                  >
                    Historia
                  </Link>
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute right-0 top-full mt-2 w-full bg-white shadow-lg border border-gray-200 z-50">
          <div className="py-2 px-4">
            <Link
              href="/"
              className="block text-[#18212D] hover:text-[#18212D] py-2"
              onClick={toggleMenu}
            >
              {t("NB001")}
            </Link>
            <button
              onClick={toggleNosotros}
              className="w-full text-left text-[#18212D] hover:text-[#18212D] py-2 flex items-center justify-between"
            >
              {t("NB002")}
              <svg
                className={`w-4 h-4 transition-transform duration-200 ${
